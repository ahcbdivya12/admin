...
<div class="content-box">
	<?php
if($_GET['subcat']=='add-home-content-jounral'){

	if(!empty($_GET['edit'])){

   $editId= $_GET['edit'];
   $query="SELECT * FROM jourrul WHERE id=$editId";
   $res= $conn->query($query);
   $editData=$res->fetch_assoc();
   $contentSection= $editData['id'];
   $firstTitle= $editData['person_name'];
   $secondTitle= $editData['dairy'];
...
<?php }elseif (!empty($_GET['view'])) {?>
  ...
  $id= $_GET['view'];
   $query="SELECT * FROM jourrul WHERE id=$id";
   $res= $conn->query($query);
   $viewData=$res->fetch_assoc();
   $backId=$viewData['id']-1;
    $firstTitle=$viewData['person_name'];
    $secondTitle=$viewData['dairy'];
		...
		<table class="table">
			<tr>
				...
				<th>Dairy</th>
						...
						<?php
  $sql1="SELECT * FROM jourrul ORDER BY id DESC";
